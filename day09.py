...
a delicate ecosystem; you might as well take some ecological readings while you wait. Maybe you can report any environmental instabilities you find to someone so the oasis can be around for the next sandstorm-worn traveler.
...
sequence of differences that you generate will be 3 3 3 3 3. Note that this sequence has one fewer value than 
the input sequence because at each step it considers two numbers from the input. Since these values aren't all 
zero, repeat the process: the values differ by 0 at each step, so the next sequence is 0 0 0 0. This means you 
have enough information to extrapolate the history! Visually, these sequences can be arranged like this:

0   3   6   9  12  15
  3   3   3   3   3
    0   0   0   0
To extrapolate, start by adding a new zero to the end of your list of zeroes; because the zeroes represent 
differences between the two values above them, this also means there is now a placeholder in every sequence above 
it:
...
def prob1():
    histories = getHistories("day09_input.txt")
    newNums = []
    for h in histories:
        num = getNewNum(h)
        newNums.append(num)
...
def getNewNum(arr):
    ...
    nextSeq = getNextSeq(arr)
    return getNewNum(nextSeq) + arr[-1]


def isZeros(arr):
    # returns true if array is entirely made of zeros
    for n in arr:
        if n != 0:
            return False
    return True
...
def getNextSeq(arr):
    # get difference between all values of arr
    seq = []
    for i in range(1, len(arr)):
        seq.append(arr[i] - arr[i-1])
    return seq


def getHistories(file):
    histories = []
    with open(file) as f:
        lines = f.readlines()
        for line in lines:
            h = [int(num) for num in line.split()]
            histories.append(h)
    return histories
...
def getPrevNum(arr):
    ...
    nextSeq = getNextSeq(arr)
    return arr[0] - getPrevNum(nextSeq)
