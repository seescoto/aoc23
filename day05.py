...
of soil to use with each kind of seed, what type of fertilizer to use with each kind of soil, what type 
...
map: describes how to convert a seed number (the source) to a soil number (the destination). This lets 
...
you know that seed number 98 corresponds to soil number 50 and that seed number 99 corresponds to soil 
number 51.
...
of the initial seeds. To do this, you'll need to convert each seed number through other categories until 
...
def prob1():
    seeds, maps = readInput("day05_input.txt")
    # find lowest location number - since thats last transformation map, just transform all through
    # all maps and then return min of the list of seeds
    for m in maps:
        for i in range(len(seeds)):
            seeds[i] = m.transform(seeds[i])
...
def readInput(fileName):
    ...
    with open(fileName) as f:
        ...
        seeds = seeds[1:]  # dont include string that says 'seeds:'
        seeds = [int(i) for i in seeds]
        # now find maps while we still have lines to read
        line = f.readline()
        while (line):
            # get info for map
            mapString = ""
...
class Map:
    def __init__(self, lines):
        # given lines (array of strings) in the form of
        # BLANK1-to-BLANK2 map:
        # [then unknown number of lines of]
        # num1 num2 num3
        lines = lines.split("\n")
        self.name = lines[0].replace(":", "")
        self.mappers = self._setMappers(lines[1:])

    def _setMappers(self, lines):
        # given
        mappers = []
        # lines.split() will make a string array of the three nums, delimiter is all whitespace
        for (line) in lines:
            line = line.split()
            if (len(line) == 3):
                m = Mapper(line)
    ...
    def transform(self, num):
        for m in self.mappers:
            ...
            if (newNum != num):
                return newNum
        # else none of the mappers changed the value
        return num
...
class Mapper:
    # given a start of range, and end of range, and a number to add to all numbers in range,
    # create a class that stores that info
    def __init__(self, arr):
        arr = [int(i) for i in arr]  # int array instead of string array
        self.range = arr[2]
        self.dests = (arr[0], arr[0] + self.range - 1)
        self.sources = (arr[1], arr[1] + self.range - 1)
        # self.dests = range(arr[0], arr[0] + self.range)
        # self.sources = range(arr[1], arr[1] + self.range)

    def map(self, num):
        # find index that the number is in the sources array, then
        # gets the element at that index in the destinations array
        if (self.sources[0] <= num <= self.sources[1]):
            i = num - self.sources[0]
            num = self.dests[0] + i
        return num

    def __str__(self):
        return "source: " + str(self.sources[0]) + "\tdestination: " + str(self.dests[0]) + "\trange: " + str(self.range)

    def __lt__(self, otherMapper):
        return otherMapper.sources[0] > self.sources[0]
...
to soil 84, fertilizer 84, water 84, light 77, temperature 45, humidity 46, and location 46. So, the 
...
def prob2():
    ...
    # make seed ranges list
    seedRanges = []
    while (seeds):
        rangeLength = seeds.pop(-1)
        source = seeds.pop(-1)
        seedRanges.append((source, rangeLength))

    hash = {}
    minScore = sys.maxsize
    for (source, rangeLength) in seedRanges:
        # too big to iterate thru all of them, try checking in steps
        # if we find the new min, test all values in between last step and new step
        # (step +1 is will never be 0, even if rangeLength < 1000)
        step = int(rangeLength/1000) + 1
        for i in range(source, source + rangeLength, step):
            if (hash.get(i, None) == None):
                hash[i] = getTransformation(i, maps)
                # if is a new min, test all vals around it to make sure we didnt miss the real min
                if (hash[i] < minScore):
                    minScore = min(hash[i], getMinInSteps(
                        i, step, maps, source, rangeLength))

    return minScore


def getMinInSteps(current, step, maps, start, rangeLength):
    # given a step, start at curr - step and end at curr + step UNLESS is out of range
    begin = (current - step) if (current - step) >= start else start
    end = (current + step) if (current + step) < (start +
                                                  rangeLength) else (start + rangeLength)

    minScore = sys.maxsize
    for i in range(begin, end):
        minScore = min(minScore, getTransformation(i, maps))

    return minScore


def getTransformation(num, maps):
    for m in maps:
        num = m.transform(num)
    return num
