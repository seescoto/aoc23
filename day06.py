...
race and also the best distance ever recorded in that race. To guarantee you win the grand prize, you need
...
spend at the beginning of the race holding down the button, the boat's speed increases by one millimeter 
per millisecond.

So, because the first race lasts 7 milliseconds, you only have a few options:

Don't hold the button at all (that is, hold it for 0 milliseconds) at the start of the race. The boat 
won't move; it will have traveled 0 millimeters by the end of the race.
...
Hold the button for 2 milliseconds, giving the boat a speed of 2 millimeters per millisecond. It will 
then get 5 milliseconds to move, reaching a total distance of 10 millimeters.
...
Hold the button for 7 milliseconds. That's the entire duration of the race. You never let go of the 
button. The boat can't move until you let go of the button. Please make sure you let go of the button so 
...
def prob1():
    ...
    # convert all numbers to their numeric value, not a string
    ...
    for i in range(len(times)):
        waysToWin = 0
        for secondsHolding in range(1, times[i]):
            speed = secondsHolding
            timeToRace = times[i] - secondsHolding
            # if we traveled more than the previous record, we won
            if (timeToRace * speed > distances[i]):
                waysToWin += 1
        product *= waysToWin
...
got earlier actually just has very bad kerning. There's really only one race - ignore the spaces between 
...
def prob2():
    # get times and distances - combine all string vals and then convert to number
    with open("day06_input.txt") as f:
        times = f.readline().split()
        distances = f.readline().split()
    time = ""
    ...
    waysToWin = 0
    for secondsHolding in range(1, time):
        speed = secondsHolding
        timeToRace = time - secondsHolding
        # if we traveled more than the previous record, we won
        if (timeToRace * speed > distance):
            waysToWin += 1
